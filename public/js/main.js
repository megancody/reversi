		...
		for(column = 0; column < 8; column++){
			...
			if (old_board[row][column] != board[row][column]){
				if(old_board[row][column] == '?' && board[row][column] == ' '){
					...
					$('#'+row+'_'+column).html('<img src="assets/images/purple_to_blue.gif" alt="blue square"/>');
				}
				else if(old_board[row][column] == 'b' && board[row][column] == 'p'){
					$('#'+row+'_'+column).html('<img src="assets/images/blue_to_purple.gif" alt="purple square"/>');
				}
				else {
...
socket.on('game_over',function(payload){
	...
	/* Check for a good game_over_response */
	if(payload.result == 'fail'){
		console.log(payload.message);
		alert(payload.message);
		return;
